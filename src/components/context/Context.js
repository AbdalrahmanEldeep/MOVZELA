...
const ContextData = ({ children }) => {
  ...
  let get__data = async () => {
    let FetchAxios = await axios.get(
      `https://api.themoviedb.org/3/movie/${dataPath}?api_key=8d06129e09b7dfab7831b59a09bd99ae&query=black&language=en-US&page=${page}`
    );
  ...
  let search__inp = async (val) => {
    if (val === "") {
      ...
      let FetchAxios = await axios.get(
        `https://api.themoviedb.org/3/search/movie?api_key=8d06129e09b7dfab7831b59a09bd99ae&query=${val}&language=en-US&page=${page}&include_adult=false`
      );
      setData(FetchAxios.data.results);
      setPages(FetchAxios.data.total_pages);
    }
